...
// 결론, 새로운 배열을 생성한다.
//데이터와 연동을 해서 생각.
//리스트의 요소를 출력할 때 , key라는 부분이 필요.
// 마치, 데이터베이스 인덱스 개념과 비슷.
// 인덱스 있으면, 인덱스를 기준으로 건색해서 빠름.
// 만약, 인덱스 없으면, 풀 스캔을 해야함. 작업의 효율성부분이 문제됨.
...
const DataListKeyAddDalTest = () => {
  // test: 상태값 타입 : 배열, 요소의 타입  : 객체
  //초기값으로 배열로 기존 4개요소 넣고, 더미 데이터
  ...
  const onRemoveText = (id) => {
    //item.id !== id -> 다르면 찹, 같으면 거짓
    //item.id == id -> 같으면 참, 다르면 거짓
    // filter(콜백함수(조건))
    // filter 조건이 참이되는 요소만 뽑아서 새로운 배열을 만든다.
    // 예) 원본 id: 1~4 , 삭제할 아이디가 : 3번
    // 1~4 검사
    // 1 !== 3 : true -> 1 필터됨, 결과배열 [1]
    // 2 !== 3 : true -> 2 필터됨, 결과배열 [1, 2]
    // 3 !== 3 : false -> 3 필터됨, 결과배열 [1, 2], 포함 안됨
    // 4 !== 3 : true -> 3 필터됨, 결과배열 [1, 2, 4]
...
export default DataListKeyAddDalTest;
