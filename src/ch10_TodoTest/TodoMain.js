...
const TodoMain = () => {
  ...
  // 추가로 의존성 배열 모양을 빈대열[], 함수 생성을 1회만 했다는 점

  //방법2) 성능상 큰 차이 없고, 규모가 커지고, 상태를 관리해주는 라이브러리 Redux사용하게되면
  //useReducer를 이용하게 됩니다.

  // 순서1) useReducer 를 임포트
  // import  { useReducer} from "react";
  //준비물, 1) 리듀서 함수, 2) useReducer 생성 3) dspatch 함수를 호출.

  //페이징, 한번에 데이터 전부 다 불러오는게 아니라, 눈에 보이는 정도의 크기만 불러옵니다.
  // 예를 들어 ) 현재, 10000개 데이터를 다 불러오고 있음.
  // 하지만, 실제로 스크롤을 내리지 않는 이상, 한번에 전체 데이터를 다 볼 수 없음.
  // 결론, 일부분만 봄. 그래서, 9개만 본다고 치면, 나머지 9991개는 불필요한 데이터 트래픽이 발생함.
  //내가 볼 수 있는 양만큼 잘라서, 불러오자.
  //준비물)
  // 1)모듈 설치하기
  // yarn add react-virtualized 설치
  // npm install react-virtualized 설치
  //2) 각 아이템요소의 높이를 구해야함. 첫번째 요소 말고, 두번째 요소부터의 높이를 구하기.
  // 왜냐하면? 첫번째 항목은 테두리가 없어서,
  // 첫번째 높이 56px  두번째 부터 57px, TodoList의 사이즈 : 512X513(57*9=513)
  //3) TodoList 이동하기.
  ...
  const onInsert = useCallback(
    (text) => {
      ...
      // 성능 개선 2번째
      // 기존과의 차이점이, useCallback, 의존선 배열이 값이 변경시 마다, 새로 함수를 생성하는 부분을
      // 방법1), 기존의 값으로 변경하는 것 -> 함수 형태로 변경.
  ...
  const onRemove = useCallback(
    (id) => {
      ...
      //성능개선 2번째, 함수 형태로 변경하고, 의존성 배열에서 ,todos 참조 안하기.
      // 결론, 새롭게 매번 함수 생성을 안함.
